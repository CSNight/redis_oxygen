<template>
    <div>
        ...
        <div class="head-container">
            <!-- 搜索 -->
            <el-input v-if="rights('MENU_QUERY')" clearable v-model="query.name" placeholder="输入文件名称搜索"
                      style="width: 200px;" size="mini"
                      class="filter-item"/>
            <el-select v-if="rights('MENU_QUERY')" v-model="query.backup_type" clearable placeholder="状态"
                       class="filter-item"
                       style="width: 90px"
                       size="mini" value="">
                <el-option v-for="item in backupTypes" :key="item.value" :label="item.label"
                           :value="item.value"/>
            </el-select>
            <el-button class="filter-item" size="mini" type="success" icon="el-icon-search"
                       v-if="rights('MENU_QUERY')">搜索
            </el-button>
            <el-button v-if="rights('MENU_QUERY')" type="danger" @click="loadBackups" icon="el-icon-refresh"
                       size="mini"/>
        </div>
        <el-scrollbar style="height: 100%">
            <el-table
                ...
                <el-table-column align="center" label="操作" width="200px">
                    <template slot-scope="scope">
                        <el-button
                                v-if="rights('BACKUP_DOWNLOAD')"
                                type="primary"
                                @click="downloadBack(scope.row.id)"
                                icon="el-icon-download"
                                size="mini">
                        </el-button>
                        <el-button
                                v-if="rights('BACKUP_DEL')"
                                type="danger"
...
<script>
    import {deleteBackupRecord, getBackupAll, getBackupList} from "../../../api/redismanage/redis_backup";
